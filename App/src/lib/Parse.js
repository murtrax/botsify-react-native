...
export class Parse extends Backend {
  ...
  initialize (token) {
    ...
    if (!_.isNull(token) && !_.isUndefined(token.sessionUserId)) {
      this._sessionUserId =
        _.isNull(token) ? null : token.sessionUserId
    }
    // console.log(this._sessionUserId)
  ...
  async getProfile () {
    ...
    return await this._fetch({
      method: 'GET',
      url: '/v1/users/' + this._sessionUserId
    })
     .then((response) => {
